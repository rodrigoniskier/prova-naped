...
def submeter_questao(request):
    """
    Esta view gerencia a página de submissão de questões.
    """
...
def get_componentes_por_periodo(request):
    """
    API que lida com um único ID ('periodo_id') ou múltiplos IDs ('periodo_ids').
    """
...
def get_historico_questoes(request):
    """
    API para o JavaScript: Retorna o histórico de questões de um componente.
    """
...
def pagina_gerar_prova(request):
    """
    Renderiza a página para o administrador selecionar os parâmetros da prova.
    """
    periodos = Periodo.objects.all()
    context = {
        'periodos': periodos
...
def prova_gerada_view(request):
    """
    Gera a prova com questões de múltiplos componentes.
    """
    ...
    componentes_da_prova = ComponenteCurricular.objects.filter(id__in=componentes_ids)
...
def prova_gabarito_view(request):
    """
    Gera a folha de gabarito para uma prova específica.
    """
